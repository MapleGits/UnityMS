	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (validcode) {
			...
			if (type == 0) {
				...
				c.getPlayer().modifyCSPoints(2, item);
			} else if (type == 3) {
				c.getPlayer().modifyCSPoints(0, item);
				c.getPlayer().modifyCSPoints(2, (item/5000));
			} else if (type == 4) {
				MapleInventoryManipulator.addById(c, item, (short) 1, "An item was obtain from a coupon.");
				c.getSession().write(MaplePacketCreator.showBoughtCSItem(item));
			} else if (type == 5) {
				c.getPlayer().modifyCSPoints(0, item);
			}
