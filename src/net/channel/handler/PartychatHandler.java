...
public class PartychatHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (!CommandProcessor.getInstance().processCommand(c, chattext)) {
            if (StringUtil.countCharacters(chattext, '@') > 4 || StringUtil.countCharacters(chattext, '%') > 4
                    || StringUtil.countCharacters(chattext, '+') > 6 || StringUtil.countCharacters(chattext, '$') > 6
                    || StringUtil.countCharacters(chattext, '&') > 6 || StringUtil.countCharacters(chattext, '~') > 6) {
                chattext = "I suck, big time. Don't listen to anything I say.";
            }
